...
MAX_HOLE_DIST            = 0.2      # Max depth below plane for hole points (m)
MIN_BOUNDARY_DIST_THRESH = 0.01      # Min XY distance to hole for boundary detection (m)
...
def process_file(file_path, output_dir):
    ...
    labels = db.fit_predict(hole_xy) if hole_xy.size else np.array([])
    ...
    print(f"[{base}] Saved boundary points ({len(boundary_pts)} pts) to: {out_csv}")


if __name__ == "__main__":
    root = tk.Tk(); root.withdraw()
    input_dir = filedialog.askdirectory(title="Select input directory with point cloud files (CSV or PLY)")
    if not input_dir:
        print("No input directory selected. Exiting.")
        exit(0)

    output_dir = filedialog.askdirectory(title="Select output directory for boundary CSVs")
    if not output_dir:
        print("No output directory selected. Exiting.")
        exit(0)

    os.makedirs(output_dir, exist_ok=True)
    files = [f for f in os.listdir(input_dir) if f.lower().endswith(('.csv', '.ply'))]
    if not files:
        print("No point cloud files found in input directory.")
        exit(0)

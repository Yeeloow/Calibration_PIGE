...
MIN_DEPTH = 0.02          # meters (min depth below plane)
...
if __name__ == '__main__':
    ...
    # Select input directory (folder) containing point clouds
    input_dir = filedialog.askdirectory(
        title="Select input directory with point cloud files (CSV or PLY)"
    )
    if not input_dir:
    ...
    for fname in sorted(os.listdir(input_dir)):
        ...
        try:
            ...
            print(f"[{base}] Failed to load file: {e}")
            continue

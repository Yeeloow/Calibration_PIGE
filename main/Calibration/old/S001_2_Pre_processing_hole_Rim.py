...
MIN_HOLE_DIST            = 0.03      # Min depth below plane for hole points (m)
MAX_HOLE_DIST            = 0.15       # Max depth below plane for hole points (m)

# Boundary detection thresholds
MIN_BOUNDARY_DIST_THRESH = 0.01      # **NEW** Min XY distance to hole for boundary detection (m)
...
csv_path = filedialog.askopenfilename(
    title="Select point cloud CSV file",
...
print(f"Plane points: {len(plane_pts)} pts")
...
print(f"Points on/below plane: {len(pts_below)} pts")
...
hole_xy     = hole_pts[:, :2]
print(f"Hole points (before filtering): {len(hole_pts)} pts")
...
good_labels  = [
    lbl for lbl in set(labels_hole)
...
hole_xy      = hole_pts[:, :2]
print(f"Hole points (after filtering): {len(hole_pts)} pts in clusters {good_labels}")
...
boundary_mask     = (
    (d_flat >= MIN_BOUNDARY_DIST_THRESH) &
    (d_flat <= MAX_BOUNDARY_DIST_THRESH)
)

boundary_pts      = plane_pts[boundary_mask]
print(
    f"Boundary points: {len(boundary_pts)} pts within "
    f"{MIN_BOUNDARY_DIST_THRESH}-{MAX_BOUNDARY_DIST_THRESH} m of hole edge"
)

# ─── 7. Create Open3D clouds ───────────────────────────────────────────
plane_cloud    = o3d.geometry.PointCloud(); plane_cloud.points    = o3d.utility.Vector3dVector(plane_pts)
plane_cloud.paint_uniform_color([1,1,0])  # yellow

hole_cloud     = o3d.geometry.PointCloud(); hole_cloud.points     = o3d.utility.Vector3dVector(hole_pts)
hole_cloud.paint_uniform_color([1,0,0])   # red

boundary_cloud = o3d.geometry.PointCloud(); boundary_cloud.points = o3d.utility.Vector3dVector(boundary_pts)
boundary_cloud.paint_uniform_color([0,0,1])  # blue

# ─── 8. Save boundary points via file dialog ───────────────────────────
save_path = filedialog.asksaveasfilename(
    title="Save boundary points as",
    defaultextension=".csv",
    initialfile=os.path.splitext(os.path.basename(csv_path))[0] + '_boundary.csv',
    filetypes=[("CSV files","*.csv")]
)
if save_path:
...
print("Displaying plane, hole, and boundary points...")

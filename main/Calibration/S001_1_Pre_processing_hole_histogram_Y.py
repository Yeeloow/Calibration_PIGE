...
def filter_points_by_plane(file_path, output_path, choice, min_z, max_z, idx):
    border_margin_Y_upper = 12
    border_margin_Y_low = 10
    border_margin_X_right = 1.5 + (idx-1)
    border_margin_X_left = 12 - (idx-1)
...
def main():
    ...
    input_folder = filedialog.askdirectory(title="Select the folder containing the point cloud CSV files")
    if not input_folder:
